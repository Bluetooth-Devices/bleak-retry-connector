...
@pytest.mark.asyncio
async def test_establish_connection_clears_bluez_state_between_retries():
    """clear_cache is called between transient failures to wipe stale BlueZ state."""
    attempts = 0
    ...
    with patch("bleak_retry_connector.clear_cache", clear_cache_mock):
        client = await establish_connection(FakeBleakClient, device, "test")

    assert isinstance(client, FakeBleakClient)
    # Eight failed attempts before success → eight clear_cache calls
    assert clear_cache_mock.await_count == 8
    clear_cache_mock.assert_awaited_with("AA:BB:CC:DD:EE:FF")


@pytest.mark.asyncio
async def test_establish_connection_clear_cache_noop_does_not_log(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """When clear_cache returns False (non-Linux / no-op), no debug log is emitted."""
    attempts = 0
    ...
    with (
        patch("bleak_retry_connector.clear_cache", AsyncMock(return_value=False)),
        caplog.at_level("DEBUG", logger="bleak_retry_connector"),
    ):
        client = await establish_connection(FakeBleakClient, device, "test")

    assert isinstance(client, FakeBleakClient)
    assert "Cleared stale BlueZ state" not in caplog.text


@pytest.mark.asyncio
async def test_establish_connection_clear_cache_logs_when_successful(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """When clear_cache returns True the debug log surfaces the address."""
    attempts = 0
    ...
    with (
        patch("bleak_retry_connector.clear_cache", AsyncMock(return_value=True)),
        caplog.at_level("DEBUG", logger="bleak_retry_connector"),
    ):
        await establish_connection(FakeBleakClient, device, "test")

    assert "Cleared stale BlueZ state after connection failure" in caplog.text
    assert "AA:BB:CC:DD:EE:FF" in caplog.text


@pytest.mark.asyncio
async def test_establish_connection_clear_cache_skipped_when_retries_exhausted():
    """When MAX_TRANSIENT_ERRORS raises, the retry tail (and clear_cache) is skipped."""
    attempts = 0
    ...
    with (
        patch("bleak_retry_connector.wait_for_disconnect", AsyncMock()),
        patch("bleak_retry_connector.calculate_backoff_time", return_value=0),
        patch("bleak_retry_connector.clear_cache", clear_cache_mock),
        pytest.raises(BleakConnectionError),
    ):
        await establish_connection(FakeBleakClient, device, "test")

    # _raise_if_needed exits before reaching the retry tail, so the last
    # failing attempt does not call clear_cache.
    assert clear_cache_mock.await_count == MAX_TRANSIENT_ERRORS - 1
